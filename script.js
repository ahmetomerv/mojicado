...
function updateForm() {
  ...
  // Add emoji generate button event listener
  const generateEmojisBtn = document.getElementById("generateEmojis");
  if (generateEmojisBtn) {
    generateEmojisBtn.addEventListener("click", generateRandomEmojis);
  }
  
  // Add color generate button event listener
  const generateColorBtn = document.getElementById("generateColor");
  if (generateColorBtn) {
    generateColorBtn.addEventListener("click", generateRandomColor);
  }
  
  // Add credits modal event listeners
  ...
  if (closeCreditsModal) {
    closeCreditsModal.addEventListener("click", closeCreditsModalFunc);
  }
  ...
  document.addEventListener("keydown", function(e) {
    if (e.key === "Escape" && creditsModal && creditsModal.classList.contains("show")) {
      closeCreditsModalFunc();
    }
  });
}
...
function generateRandomEmojis() {
  // Add dice rolling animation to the button
  const generateBtn = document.getElementById("generateEmojis");
  if (generateBtn) {
    generateBtn.classList.add("dice-rolling");
    
    // Remove the animation class after animation completes
    setTimeout(() => {
      generateBtn.classList.remove("dice-rolling");
    }, 600);
  }

  // Array of popular emojis for variety
  const emojis = [
    "😀", "😃", "😄", "😁", "😆", "😅", "😂", "🤣", "😊", "😇",
    "🙂", "🙃", "😉", "😌", "😍", "🥰", "😘", "😗", "😙", "😚",
    "😋", "😛", "😝", "😜", "🤪", "🤨", "🧐", "🤓", "😎", "🤩",
  ...
  // Update the emoji input field
  const emojiInput = document.getElementById("emojiPicker");
  if (emojiInput) {
    emojiInput.value = randomEmojis;
    
    // Trigger the setColor function to update the wallpaper
    // Since this is an emoji change, we should trigger a render
    clearTimeout(timer);
    timer = setTimeout(render, 300);
  }
}
...
function generateRandomColor() {
  // Add paintbrush animation to the button
  const generateBtn = document.getElementById("generateColor");
  if (generateBtn) {
    generateBtn.classList.add("paintbrush-animation");
    
    // Remove the animation class after animation completes
    setTimeout(() => {
      generateBtn.classList.remove("paintbrush-animation");
    }, 600);
  }
...
function closeCreditsModalFunc() {
  const modal = document.getElementById("creditsModal");
  if (modal) {
